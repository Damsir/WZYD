...
@property(nonatomic,strong) ReceiveViewController *receiveVC;
@property(nonatomic,strong) SendOutViewController *sendOutVC;
...
- (void)viewDidLoad {
    ...
    //self.navigationItem.title = @"在办公文";
    [self initNavigationBarTitle:@"在办公文"];
...
- (void)screenRotation:(NSNotification *)noti
{
    ...
    _receiveVC.view.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    _sendOutVC.view.frame = CGRectMake(SCREEN_WIDTH, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
   
    [_receiveVC screenRotation];
    [_sendOutVC screenRotation];
}
...
-(void)initDifferentControllers
{
    ...
    receiveVC.view.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    receiveVC.nav = self.navigationController;
    _receiveVC = receiveVC;
    [_scrollView addSubview:receiveVC.view];
    
    SendOutViewController *sendOutVC = [[SendOutViewController alloc] init];
    sendOutVC.view.frame = CGRectMake(SCREEN_WIDTH, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    sendOutVC.nav = self.navigationController;
    _sendOutVC = sendOutVC;
    [_scrollView addSubview:sendOutVC.view];
}
