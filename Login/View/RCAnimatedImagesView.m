...
- (void)_init
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (MainR.size.width == 414) {
           imageView = [[UIImageView alloc] initWithFrame:CGRectMake(-imageViewsBorderOffset, -imageViewsBorderOffset+70, self.bounds.size.width + (imageViewsBorderOffset*1.5), self.bounds.size.height+imageViewsBorderOffset)];
        }
        else
        {
         imageView = [[UIImageView alloc] initWithFrame:CGRectMake(-imageViewsBorderOffset, -imageViewsBorderOffset+50, self.bounds.size.width + (imageViewsBorderOffset*1.5), self.bounds.size.height+imageViewsBorderOffset)];
        
        
        }
//        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(-imageViewsBorderOffset, -imageViewsBorderOffset+50, self.bounds.size.width + (imageViewsBorderOffset*1.5), self.bounds.size.height + (imageViewsBorderOffset * 2))];

//
        
//        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(-(imageViewsBorderOffset*3.3-MainR.size.width)*0.5, -imageViewsBorderOffset+50,(imageViewsBorderOffset*3.3-MainR.size.width)*0.5, self.bounds.size.height + (imageViewsBorderOffset * 2))];
...
- (void)bringNextImage
{
    ...
    while (nextImageToShowIndex == currentlyDisplayingImageIndex);//随机产生的索引相同时
    
    currentlyDisplayingImageIndex = nextImageToShowIndex;
    
    NSLog(@"nextImageToShowIndex:%ld",(unsigned long)nextImageToShowIndex);
    //代理提供下一张需要显示的图片
...
//        NSInteger randomTranslationValueX = imageViewsBorderOffset*3.5-[[self class] randomIntBetweenNumber:0 andNumber:imageViewsBorderOffset];
        //产生随机移动的x值
        NSInteger randomTranslationValueX = imageViewsBorderOffset*1.18-[[self class] randomIntBetweenNumber:0 andNumber:imageViewsBorderOffset];
