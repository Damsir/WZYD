    ...
    $.fn.dragList = function(options) {
        ...
        return this.each(function() {
            ...
            function mousedownHanlder(evt) {
                ...
                return;
                var sv = evt.srcElement;
                if (sv.btnFlag)
                    return;
                while (sv.tagName != 'LI') {
                    sv = sv.parentNode;
                }
                $(sv).addClass("touchActive");
            }

            function mousemoveHanlder(evt) {
                if (!startPoint)
                    return;
                if (Math.abs(startPoint.x - evt.pageX > 10) || Math.abs(startPoint.y - evt.pageY > 10)) {
                    moved = true;
                }
            }
            ...
            try {
                ...
                this.addEventListener("mousemove", mousemoveHanlder, false);
