...
-(void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data{
    ...
    self.currentlySize = self.currentSize/1024.0/1024.0;
    self.totallySize = self.totalSize/1024.0/1024.0;
    NSString *sizeRate = self.totallySize > 1 ? [NSString stringWithFormat:@"%.02f M/%.02f M", self.currentlySize,self.totallySize] : [NSString stringWithFormat:@"%.02f K/%.02f K", self.currentlySize * 1024.0,self.totallySize * 1024.0];
	[self save:data];
    ...
    if (progress==1) {
        ...
        [self.delegate download:self updateProgress:@"a" progress:progress sizeRate:sizeRate];
    }
    data = nil;
}
