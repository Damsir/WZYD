...
//    lineUpAnimation.duration = lineUpAnimation.settlingDuration;
    lineUpAnimation.beginTime= 0.30;
...
- (UIBezierPath *)getCirclePathWithProgress:(CGFloat)progress
{
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(squareW, squareH) radius:squareW startAngle: - M_PI_2 endAngle: (M_PI * 2) * progress - M_PI_2 clockwise:YES];
    return path;
}

#pragma mark -- 绘制波浪线

- (UIBezierPath *)getWavePathWithOffset:(CGFloat)offset WaveHeight:(CGFloat)height WaveCurvature:(CGFloat)curvature{
...
- (CASpringAnimation *)getDownloadAnimationShow:(BOOL)isShow{
...
#pragma mark -- 进度百分百label
- (CGRect)getProgressRect{
    CGFloat SW = CGRectGetWidth(self.viewRect);
    return CGRectMake(midPointX, SW/2+waveHeight*2, lineW, SW/4);
}
#pragma mark -- 下载大小label
- (CGRect)getDownloadRect{
    CGFloat SW = CGRectGetWidth(self.viewRect);
    return CGRectMake(midPointX, SW/2-SW/4, lineW, SW/4);
}
