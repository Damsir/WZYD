...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.isClicked=NO;
        self.clickFadeColor=[UIColor colorWithRed:103.0/255.0 green:216.0/255.0 blue:198.0/255.0 alpha:100];
...
-(void)setbuttonTitle:(NSString *)title fontSize:(CGFloat)size fontStyle:(NSString *)style fontColor:(UIColor*)titleColor Alignment:(NSInteger)alignment
{
    ...
    self.titleLabel.font=[UIFont fontWithName:style size:size];
...
-(void)buttonClickedAnimation:(BOOL)animation WithFadeColor:(UIColor *)fadeColor
{
    ...
    if (animation==YES) {
        self.backgroundColor=fadeColor;
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (_clickFadeColor!=nil) {
        [self buttonClickedAnimation:YES WithFadeColor:_clickFadeColor];
    }
    self.isClicked=!self.isClicked;
//    if (self.isClicked==YES&&self.selectedImage!=nil) {
...
#pragma -mark Customize the specific button －copy from Oskar Groth on 2013-09-29.
...
{
    self.isClicked=!self.isClicked;
    if (self.imageView.image==self.normalImage&&self.selectedImage!=nil) {
        [self setImage:self.selectedImage forState:UIControlStateNormal];
    }
    else if (self.imageView.image==self.selectedImage&&self.normalImage!=nil)
    {
        [self setImage:self.normalImage forState:UIControlStateNormal];
    }
    else
