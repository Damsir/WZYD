...
@implementation MeetingCalendarItem
...
-(void)selectedDate:(UIButton *)dayBtn
{
    ...
    NSString *dateString = [NSString stringWithFormat:@"%ld/%ld/%ld",(long)_selectedYear,(long)_selectedMonth,(long)_selectedDay];
    // NSLog(@"dateString::%@",dateString);
    for (int i=0; i<_meetingData.count; i++)
    {
        MeetingRoomModel *model = _meetingData[i];
        NSString *meetingTime = model.timeStart;
        NSRange range = [meetingTime rangeOfString:@" "];
        meetingTime = [meetingTime substringToIndex:range.location];
